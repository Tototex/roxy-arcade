...
class Roxy_Arcade {
  ...
  const OPTION_LAST_AWARDED_MONTH = 'roxy_arcade_last_awarded_month'; // "YYYY-MM"
  ...
  public static function enqueue_assets() {
    ...
    // Subtitle text: explicitly white in CSS; this text is now without "(site timezone)"
    ...
    <div class="roxy-arcade">
      <div class="roxy-arcade__left">
        <div class="roxy-arcade__header">
          <!-- Single title only (no duplicate icon/title row) -->
          <h2 class="roxy-title">Roxy Arcade</h2>
          <div class="roxy-subtitle" id="roxyAwardRule"></div>
        </div>

        <!-- Tabs are built dynamically from registered games -->
  ...
  public static function rest_post_score(WP_REST_Request $req) {
    ...
    // Rate limit: 1 submission per game per 10 seconds per user
  ...
  /**
   * Monthly snapshot award (unchanged behavior).
   */
  ...
  public static function admin_menu() {
    add_options_page('Roxy Arcade', 'Roxy Arcade', 'manage_options', 'roxy-arcade-settings', [__CLASS__, 'settings_page']);
  }
